package com.bontsi.ndiraphutcha.common;
...
public class StateMaintainer {
	 ...
	 * Constructor
	 */
	public StateMaintainer(FragmentManager fragmentManager,
			String stateMaintainerTAG) {
	...
	public boolean firstTimeIn() {
		try {
			...
			mStateMaintainerFrag = (StateMngFragment) mFragmentManager.get()
					.findFragmentByTag(mStateMaintenerTag);

			// Criando novo RetainedFragment
			if (mStateMaintainerFrag == null) {
				Log.d(TAG, "State Maintainer is empty " + mStateMaintenerTag);
				...
				Log.d(TAG, "State already exist " + mStateMaintenerTag);
				mIsRecreating = true;
				return false;
			}
		} catch (final NullPointerException e) {
			Log.w(TAG, "Error on firstTimeIn()");
			return false;
		}
	}

	/**
	 * Return <strong>true</strong> it the current Activity was recreated at
	 * least one time
	 * 
	...
	/**
	 * Fragment resposible to preserve objects. Instantiated only once. Uses a
	 * hashmap to save objs
	 */
	public static class StateMngFragment extends Fragment {
		private final HashMap<String, Object> mData = new HashMap<String, Object>();
