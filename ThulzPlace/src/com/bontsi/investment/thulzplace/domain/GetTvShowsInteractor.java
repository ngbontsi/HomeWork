...
/**
 * GetTvShows interactor implementation. This interactor will go out of the UI
 * thread using the executor, then will get a list of TvShows from the Catalog
 * and will return the result over the main thread using a callback and
 * MainThread dependency.
 * 
...
class GetTvShowsInteractor implements Interactor, GetTvShows {
	...
	@Inject
	GetTvShowsInteractor(Catalog catalog, Executor executor,
	...
	public void run() {
		...
		if (haveToShowError()) {
			...
			final Collection<TvShow> tvShows = catalog.getTvShows();
