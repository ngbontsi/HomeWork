 ...
 */
package com.bontsi.investment.thulzplace.ui.activity;
...
import com.bontsi.investment.thulzplace.di.ActivityContext;
import com.bontsi.investment.thulzplace.domain.tvshow.TvShow;

/**
 * Class created to handle all the navigation between activities. This class
 * knows how to open every activity in the application and provides to the
 * client code different methods to start activities with the information
 * needed.
 * <p/>
 * You can use other approach based on ActionCommands if you wish. To do that
 * review MVVM pattern and ActionCommand interface.
 * 
...
public class Navigator {
	...
	/*
	 * This method contains the key of the application navigation. If there are
	 * no fragments attached we will launch TvShowActivity.
	 * 
	 * If any fragment is visible on injected activity, we will load the TvShow.
	 * 
	 * Other approach to connect fragments could be based on a Bus event
	 * implementation. But this is only valid if you only have fragments in your
	 * activity.
	 */
	...
	private boolean canInteractWithFragments() {
		tvShowFragment = (TvShowFragment) getFragmentManager()
				.findFragmentById(R.id.f_tv_show);
		tvShowDraggableFragment = (TvShowDraggableFragment) getFragmentManager()
				.findFragmentById(R.id.f_tv_show_draggable);
	...
	public void openTvShowActivity(final String tvShowId) {
		final Intent intent = TvShowActivity.getLaunchIntent(activityContext,
				tvShowId);
