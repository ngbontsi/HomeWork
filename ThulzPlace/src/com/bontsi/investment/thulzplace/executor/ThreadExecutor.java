...
class ThreadExecutor implements Executor {
	...
	@Inject
	ThreadExecutor() {
		final int corePoolSize = CORE_POOL_SIZE;
		final int maxPoolSize = MAX_POOL_SIZE;
		final int keepAliveTime = KEEP_ALIVE_TIME;
		final TimeUnit timeUnit = TIME_UNIT;
		final BlockingQueue<Runnable> workQueue = WORK_QUEUE;
	...
	public void run(final Interactor interactor) {
		if (interactor == null) {
			throw new IllegalArgumentException(
					"Interactor to execute can't be null");
		}
